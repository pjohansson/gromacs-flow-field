...
void pr_inputrec(FILE* fp, int indent, const char* title, const t_inputrec* ir, gmx_bool bMDPformat)
{
    ...
    if (available(fp, ir, indent, title))
    {
        ...
        if (!bMDPformat)
        {
            pr_rvecs(fp, indent, "posres-com", as_vec_array(ir->posres_com.data()), ir->posres_com.size());
            pr_rvecs(fp, indent, "posres-comB", as_vec_array(ir->posres_comB.data()), ir->posres_comB.size());
        }
        /* ***** */
