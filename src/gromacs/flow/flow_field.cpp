...
FlowField::FlowField(const std::string& fnbase,
                     const int          nx,
                     const int          nz,
                     const matrix       box)
:fnbase { fnbase },
...
void FlowField::_setup_grid_and_finalize(const int    nx,
                                         const int    nz,
                                         const matrix box)
{
                                     ...
                                     const size_t file_index)
{
    ...
    const bool allArraySizesAreEqual = (
        ...
        && (num_bins == flow_field.mass_density.size())
        && (num_bins == flow_field.temperature.size())
        && (num_bins == flow_field.ux.size())
        && (num_bins == flow_field.uz.size())
    );

    GMX_RELEASE_ASSERT(
        allArraySizesAreEqual,
